  ...
  programs.waybar = {
    ...
    settings = [{
      ...
      "memory" = {
        ...
        "states" = { "warning" = 85; }; # 当使用率超过85%时，添加 "warning" 类 (会触发上面的CSS动画)
      };
      ...
      "mpd" = {
        ...
        "on-click-middle" = "kitty --class='ncmpcpp' ncmpcpp "; # 中键单击：在kitty中打开ncmpcpp
        "on-scroll-up" = "mpc --quiet prev"; # 向上滚动：上一曲
        "on-scroll-down" = "mpc --quiet next"; # 向下滚动：下一曲
        "smooth-scrolling-threshold" = 5; # 平滑滚动阈值
        "tooltip-format" = "{title} - {artist} ({elapsedTime:%M:%S}/{totalTime:%H:%M:%S})"; # 工具提示格式
      };
